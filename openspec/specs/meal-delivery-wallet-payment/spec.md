...
After a successful customer meal-delivery wallet charge, the system MUST NOT create an Admin Wallet cash credit that increases platform balance for that same charge. Customer wallet debit, delivery payment status, and meal-payment history rules from this capability remain unchanged. (Custody cash is credited on customer recharge; see `admin-wallet-funding-custody` and `admin-wallet-meal-revenue-recognition`.)
