...
- **WHEN** files under `scripts/cron/` with a `.sh` extension are present in the repository
...
The repository MUST declare Git attributes so `*.sh` files are treated as text with `eol=lf`, preventing Windows checkouts/commits from reintroducing CRLF into production shell scripts. Scripts under `scripts/cron/` MUST remain executable (mode bit and/or installer `chmod +x`).
