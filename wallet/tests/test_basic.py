...
class WalletAPITests(APITestCase):
    ...
    def test_successful_recharge(self):
        response = self.client.post(
            self.recharge_url,
            {'amount': '500.00', 'note': 'Cash top-up'},
            format='json',
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['wallet']['balance'], '500.00')
    ...
    def test_successful_withdraw(self):
        recharge_wallet(self.profile, Decimal('500.00'))
        response = self.client.post(
            self.withdraw_url,
            {'amount': '500.00'},
            format='json',
    ...
    def test_invalid_recharge_amount(self):
        for bad in ('0', '-10', '1.234'):
            response = self.client.post(self.recharge_url, {'amount': bad}, format='json')
    ...
    def test_frozen_wallet_cannot_recharge(self):
        ...
        response = self.client.post(self.recharge_url, {'amount': '10.00'}, format='json')
        self.assertEqual(response.status_code, 400)
        wallet.refresh_from_db()
        self.assertEqual(wallet.balance, Decimal('0.00'))

    def test_withdraw_exceeds_balance(self):
        recharge_wallet(self.profile, Decimal('50.00'))
    ...
    def test_idempotency_replay_does_not_double_credit(self):
        payload = {'amount': '100.00', 'idempotency_key': 'key-abc'}
    ...
    def test_idempotency_conflict_on_different_amount(self):
        self.client.post(
            self.recharge_url,
            {'amount': '100.00', 'idempotency_key': 'key-xyz'},
            format='json',
        )
        response = self.client.post(
            self.recharge_url,
            {'amount': '200.00', 'idempotency_key': 'key-xyz'},
            format='json',
        )
    ...
    def test_transaction_list_and_detail(self):
        recharge_wallet(self.profile, Decimal('30.00'))
        withdraw_wallet(self.profile, Decimal('10.00'))
        ...
        self.assertEqual(results[0]['type'], 'withdraw')
        self.assertEqual(results[1]['type'], 'recharge')
